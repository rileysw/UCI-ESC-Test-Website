  ...
  $("#formUCINetID").submit(function() {
    event.preventDefault();
		...
		$.ajax({
          ...
          if (studentObject.Tuesday2 == null || studentObject.Tuesday2 == "") {
						$("#tuesday2").hide();
					} else {
						$("input[name=tuesday2]").val(studentObject.Tuesday2);
					}
					if (studentObject.Wednesday == null || studentObject.Wednesday == "") {
						$("#wednesday").hide();
					} else {
    ...
    console.log("pickedUpSwag3 = " + $("#pickedUpSwag3").is(':checked'));
		...
		$.ajax({
				url: 'updateGiveAwayAndEmail.php',
				type: 'POST',
		...
		event.preventDefault();
		window.location.replace("http://esc.eng.uci.edu/OLD_SITE/escan2.html");
	});
	//
